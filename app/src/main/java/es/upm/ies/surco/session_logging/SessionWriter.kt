...
object SessionWriter {
    ...
    fun createJSONHeader(
        ...
        for ((index, beacon) in beacons.withIndex()) {
            ...
            val base64encodedDescription = Base64.getEncoder()
    ...
    fun appendCsvBodyFromData(
        ...
        for (beacon in beacons) {
            ...
            for (entry in beacon.sensorData.value!!) {
                ...
                result.append(beacon.id).append(",").append(localizedTimestamp.format(formatter))
                    .append(",").append(entry.data).append(",").append(entry.latitude).append(",")
                    .append(entry.longitude).append(",").append(entry.azimuth).append("\n")
    ...
    fun appendCsvBodyFromTempFile(
        outputStreamWriter: OutputStreamWriter, tempFile: File
    ) {
